...
async def agenerate(
    ...
    # Fix provider for Qwen models on OpenRouter to use chutes/fp8
    if "openrouter" in model_name.lower() and "qwen" in model_name.lower():
        completion_kwargs["extra_body"] = {
            "provider": {
                "order": ["chutes"],
                "quantizations": ["fp8"],
        ...
        log.debug(f"Fixed provider to Chutes/fp8 for model: {model_name}")
