...
# Optional v2 imports (if files exist)
try:
    from .learnable_strategy_space_v2 import LearnableStrategySpaceV2
except ImportError:
    LearnableStrategySpaceV2 = None

try:
    from .neural_adversarial_learnable_v2_bandit import (
        LearnableAdversarialV2Config,
        NeuralAdversarialLearnableV2Bandit,
    )
except ImportError:
    LearnableAdversarialV2Config = None
    NeuralAdversarialLearnableV2Bandit = None
...
def create_bandit(
    ...
    if bandit_type == "exp3":
        ...
        if NeuralAdversarialLearnableV2Bandit is not None:
            return NeuralAdversarialLearnableV2Bandit(prompt_space, config, tensorboard_dir)
        else:
            raise ImportError("adversarial_learnable_v2 requires neural_adversarial_learnable_v2_bandit.py")
