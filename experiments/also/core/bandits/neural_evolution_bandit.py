...
class NeuralEvolutionConfig(BanditConfig):
    ...
    fitness_normalization_threshold: float = 1e6
...
class NeuralAdversarialEvolutionBandit(NeuralAdversarialBandit):
    ...
    def _initialize_populations(self) -> None:
        ...
        for agent in ["p1", "p2"]:
            ...
            if n_available <= 0:
                raise ValueError(f"Cannot initialize {agent} population: prompt space has no arms")

            population = []
            for idx in range(self.evo_config.population_size):
                source_idx = idx % n_available
                prompt_text = self.prompt_space.get_prompt(agent, source_idx)
                embedding = self.prompt_space.get_embedding(agent, source_idx).copy()
            ...
            logger.info(
                f"Initialized {agent} population with {len(population)} units "
                f"from {n_available} prompt-space arms (idx=0 is original)"
            )
    ...
    def _select_by_fitness(
        ...
        # Apply dynamic eta scheduling if enabled
    ...
    def update_with_context(
        ...
        for idx, unit in enumerate(population):
            ...
            if not np.isfinite(unit.fitness):
                logger.warning(
                    f"[Turn {turn}] Non-finite fitness for {agent} arm {idx}; resetting to 1.0"
                )
                unit.fitness = 1.0

            unit.fitness = max(float(unit.fitness), 1e-12)

        self._normalize_fitness_if_needed(agent, turn)

    def _normalize_fitness_if_needed(self, agent: Literal["p1", "p2"], turn: int) -> None:
        """Keep population fitness on a numerically stable positive scale."""
        population = self.populations.get(agent, [])
        if not population:
            return

        max_fitness = max(unit.fitness for unit in population)
        threshold = self.evo_config.fitness_normalization_threshold
        if max_fitness <= threshold:
            return

        for unit in population:
            unit.fitness = max(unit.fitness / max_fitness, 1e-12)

        logger.debug(
            f"[Turn {turn}] Normalized {agent} fitness values by max={max_fitness:.4e}"
        )
    ...
    async def _mutate_unit_async(
        ...
        try:
            ...
            if new_prompt and len(new_prompt) > 10:
                ...
                logger.warning("Invalid mutation result, keeping original prompt")
                return None
    ...
    def _log_evolution_summary(
        ...
        if n_turns > 0:
            ...
            # Traces can be shorter than the evolved population when the
            # population was initialized larger than the source prompt space.
            for trace, weight in zip(self.score_traces[agent], weights):
                limit = min(len(trace), n_arms)
                if limit > 0:
                    nn_cumulative[:limit] += np.asarray(trace[:limit]) * weight

            for trace, weight in zip(self.actual_score_traces[agent], weights):
                limit = min(len(trace), n_arms)
                if limit > 0:
                    actual_cumulative[:limit] += np.asarray(trace[:limit]) * weight
        ...
        if n_turns > 0:
            ...
            for i in range(n_arms):
                row = [str(i)]
                for t in range(n_turns - show_turns, n_turns):
                    nn_trace = self.score_traces[agent][t]
                    actual_trace = self.actual_score_traces[agent][t]
                    nn_val = nn_trace[i] if i < len(nn_trace) else 0.0
                    actual_val = actual_trace[i] if i < len(actual_trace) else 0.0
