...
class BanditConfig:
    ...
    # Greedy selection mode: directly select arm with highest NN prediction (no EXP3, no softmax)
    greedy_selection: bool = False
    selection_epsilon: float = 0.0  # Epsilon for epsilon-greedy exploration (0.0 = pure greedy)
    selection_temperature: float = 1.0  # Temperature for softmax selection

    # Textual gradient reflection parameters
    enable_reflection: bool = False
    reflection_interval: int = 10  # 最小间隔（回合数）
    reflection_buffer_size: int = 20  # 用于分析的历史记录数
    reflection_lookback: int = 5  # 用于判断方差的最近 N 轮
    reflection_model: str = "openrouter/qwen/qwen-2.5-72b-instruct"
    reflection_temperature: float = 0.7
    elite_protection_ratio: float = 0.15  # 只保护 top 15%
...
class SelectionRecord:
    ...
    opponent_response_text: str | None = None  # Opponent's actual utterance for reflection
