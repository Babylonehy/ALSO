...
def display_metrics(metrics: dict[str, Any]) -> None:
    ...
    overall_table.add_column("Std", justify="right")
    ...
    if metrics.get("p1_final_mean") is not None or metrics.get("p1_final_reward") is not None:
        ...
        final_table.add_column("Std", justify="right")
...
def display_multi_evalset_comparison(
    ...
    for evalset in results_by_evalset.keys():
        ...
        table.add_column(f"{display_name}\nGoal SE", justify="right", style="dim")
        table.add_column(f"{display_name}\nOverall ↑", justify="right", style="yellow")
        table.add_column(f"{display_name}\nOverall SE", justify="right", style="dim")
    ...
    for exp in experiments:
        ...
        for evalset, (df, dim_df) in results_by_evalset.items():
            ...
            if not exp_row.empty:
                row = exp_row.iloc[0]
                # Use average of P1 and P2 goal (consistent with All Dimensions table)
                p1_goal = row.get("p1_goal", row.get("p1_final", 0)) or 0
                p2_goal = row.get("p2_goal", row.get("p2_final", 0)) or 0
                goal = (p1_goal + p2_goal) / 2
                # SE: combine using sqrt(se1^2 + se2^2) / 2
                p1_goal_se = row.get("p1_goal_se", row.get("p1_final_se", 0)) or 0
                p2_goal_se = row.get("p2_goal_se", row.get("p2_final_se", 0)) or 0
                goal_se = ((p1_goal_se ** 2 + p2_goal_se ** 2) ** 0.5) / 2
                overall = row.get("avg_final", row.get("avg", 0))
                overall_se = row.get("avg_final_se", row.get("avg_se", 0))
                row_data.extend([
                    f"{goal:.4f}",
                    f"{goal_se:.4f}",
                    f"{overall:.4f}",
    ...
    for evalset in results_by_evalset.keys():
        display_name = evalset_display_names.get(evalset, evalset.upper())
        summary_table.add_column(f"{display_name} Goal", justify="right")
        summary_table.add_column(f"{display_name} Overall", justify="right")
    ...
    for exp in experiments:
        row_data: list[str] = []
        for evalset, (df, _) in results_by_evalset.items():
            exp_row = df[df["experiment"] == exp]
            if not exp_row.empty:
                row = exp_row.iloc[0]
                # Use average of P1 and P2 goal (consistent with All Dimensions table)
                ...
                goal = (p1_goal + p2_goal) / 2
                overall = row.get("avg_final", row.get("avg", 0))
                row_data.extend([f"{goal:.4f}", f"{overall:.4f}"])
            else:
...
def main() -> None:
    ...
    if args.csv:
        ...
        try:
            ...
            experiments = csv_df.iloc[:, 0].dropna().tolist()
