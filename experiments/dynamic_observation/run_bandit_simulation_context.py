...
from experiments.dynamic_observation.core.bandits.progressive_prompt_breeder_bandit import ProgressivePromptBreederConfig
...
MIHOMO_CONFIG_PATHS = [
    ...
    Path.home() / ".config/clash/runtime.yaml",
...
class MihomoProxyManager:
    ...
    def _wait_for_proxy_ready(self) -> bool:
        ...
        while True:
            ...
            try:
                with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
                    ...
                    if result == 0:
                        ...
                        console.print(f"[green]✓ Proxy port {self.http_port} is ready (took {elapsed:.1f}s)[/]")
                        logger.info(f"Proxy port {self.http_port} is ready after {elapsed:.1f}s")
                        return True
            ...
            if current_time - last_log_time >= log_interval:
                ...
                console.print(f"[yellow]Still waiting for proxy... ({elapsed:.0f}s elapsed)[/]")
    ...
    def _filter_proxies(self, config: dict) -> dict:
        ...
        allowed_keywords = [
            ...
            "美国", "USA", "US", "America", "Los Angeles", "Seattle", "New York", "San",
            "新加坡", "Singapore", "SG",
        ]
        # 排除的国家/地区关键词
        blocked_keywords = [
        ...
        if "proxies" in config:
            ...
            filtered_proxies = [p for p in config["proxies"] if is_allowed_proxy(p.get("name", ""))]
            config["proxies"] = filtered_proxies
            logger.info(f"Filtered proxies: {len(filtered_proxies)}/{original_count} (JP/US/SG only, no HK/free)")
                ...
                for group in config["proxy-groups"]:
                    if "proxies" in group:
                        group["proxies"] = [
                            ...
                            if p in filtered_names or p in ("DIRECT", "REJECT", "PASS")
                            or any(g["name"] == p for g in config.get("proxy-groups", []))
                        ]
    ...
    def _create_auto_select_group(self, config: dict) -> dict:
        ...
        logger.info(f"Created auto-select group with {len(proxy_names)} proxies (url-test, lowest latency)")
    ...
    def start(self) -> int:
        ...
        logger.info(f"Found available ports: HTTP={self.http_port}, SOCKS={self.socks_port}, API={self.api_port}")
        ...
        if not self._wait_for_proxy_ready():
            ...
            self.process.kill()
            raise RuntimeError(f"mihomo failed to start or proxy not responding: {stderr}")

        self._started = True
        logger.info(f"Started mihomo proxy on port {self.http_port} (PID: {self.process.pid})")
...
def create_bandit_config_from_args(args: argparse.Namespace) -> BanditConfig:
    ...
    if hasattr(args, "selection_epsilon"):
        selection_params["epsilon"] = args.selection_epsilon
    ...
    if bandit_type in ("evoprompt_ga", "evoprompt_de"):
        ...
        config = PromptBreederConfig(**base_params, **selection_params, **population_params)
        ...
        config = ProgressivePromptBreederConfig(**base_params, **selection_params, **ppb_params)
        ...
        config = BanditConfig(**base_params)
...
def migrate_episode_logs_to_new_tag(source_tag: str, target_tag: str) -> int:
    ...
    logger.info(f"Migrated {migrated_count} EpisodeLog records from '{source_tag}' to '{target_tag}'")
            ...
            if json_path.exists():
                try:
                    ...
                    if "experiment_tag" in data:
                        ...
                        logger.info(f"Updated experiment_tag in {json_file}: {old_tag} -> {new_tag}")
                except Exception as e:
...
def resolve_experiment_source(source_tag_or_path: str) -> tuple[str, Path]:
    ...
    """
    outputs_dir = PROJECT_ROOT / "experiments/dynamic_observation/outputs"
...
class BatchProgressTracker:
    ...
    def __init__(self) -> None:
        self.episodes: dict[str, dict] = {}  # scenario_id -> {turn, max_turn, agent_names, step}
        self.completed = 0
        self.total = 0
        self.successes = 0
        self.errors = 0

    def update(self, scenario_id: str, turn: int, max_turn: int, step: str = "") -> None:
    ...
    def register(self, scenario_id: str, max_turn: int, agent_names: str) -> None:
        self.episodes[scenario_id] = {"turn": 0, "max_turn": max_turn, "agent_names": agent_names, "step": "init"}
    ...
    def get_status(self) -> str:
        ...
        for scenario_id, info in list(self.episodes.items())[:3]:
            ...
            running.append(f"{name[:12]}:T{info['turn']}/{info['max_turn']}({step_short})")
    ...
    def __init__(
        ...
        if self.optimize_p1 or self.optimize_p2:
            ...
            if self.selection_mode == "strategy":
                ...
                skip_embeddings = bandit_type in ("opro", "prompt_breeder", "progressive_prompt_breeder", "evoprompt_ga", "evoprompt_de", "tpe")
                logger.info(
                    f"Initializing StrategySpace for strategy selection mode "
    ...
    async def run_episode(self) -> dict:
        ...
        with progress_context as progress:
            ...
            while not all(terminated.values()) and self.current_turn < self.max_turns:
                ...
                if self.progress_tracker is not None:
                    self.progress_tracker.update(self.scenario_id, self.current_turn, self.max_turns, step="action")
                ...
                agent_obs_pairs = [(agent, observations.get(agent.agent_name)) for agent in agents]
                action_results = await asyncio.gather(*[agent.aact(obs) for agent, obs in agent_obs_pairs])
                actions = {agent.agent_name: action for (agent, _), action in zip(agent_obs_pairs, action_results)}
                ...
                if self.progress_tracker is not None:
                    self.progress_tracker.update(self.scenario_id, self.current_turn, self.max_turns, step="env_step")
                ...
                if info:
                    ...
                    p1_complete = p1_info.get("complete_rating") if isinstance(p1_info, dict) else None
                    p2_complete = p2_info.get("complete_rating") if isinstance(p2_info, dict) else None
                    ...
                    if isinstance(p1_complete, tuple) and len(p1_complete) >= 2:
                        ...
                        if isinstance(dim_dict, dict):
                            p1_dim_rewards = {k: v for k, v in dim_dict.items() if k != "overall_score"}
                    if isinstance(p2_complete, tuple) and len(p2_complete) >= 2:
                        dim_dict = p2_complete[1]
                        if isinstance(dim_dict, dict):
                            p2_dim_rewards = {k: v for k, v in dim_dict.items() if k != "overall_score"}
                ...
                if self.progress_tracker is not None:
                    self.progress_tracker.update(self.scenario_id, self.current_turn, self.max_turns, step="update")
                    ...
                    if p1_reward == 0.0:
                        ...
                        if self.use_context_embedding and hasattr(self.bandit_p1, 'update_with_context'):
                            self.bandit_p1.update_with_context(
                                "p1", p1_idx, p1_reward, self.current_turn, context_embedding,
                    ...
                    if p2_reward == 0.0:
                        ...
                        if self.use_context_embedding and hasattr(self.bandit_p2, 'update_with_context'):
                            self.bandit_p2.update_with_context(
                                "p2", p2_idx, p2_reward, self.current_turn, context_embedding,
                ...
                if not all(terminated.values()):
                    ...
                    for agent_key in agents_to_refresh:
                        ...
                        if self.progress_tracker is not None:
                            self.progress_tracker.update(self.scenario_id, self.current_turn, self.max_turns, step="select")
...
""",
    ...
    parser.add_argument(
        ...
        type=int,
        default=100,
        help="Training epochs for neural bandits (default: 100, matching original NeuralUCB)",
    )
    parser.add_argument(
        "--update-interval",
        type=int,
    ...
    parser.add_argument(
        ...
        type=str,
        default="nn",
        choices=["nn", "actual", "mean", "mean-zscore"],
        help=(
            "How to compute cumulative scores for arm selection: "
            "'nn' = use only NN predicted scores (default), "
            "'actual' = use only actual observed scores (with importance weighting), "
            "'mean' = simple average of NN and actual scores, "
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.5,
        help="Weight for NN predictions when cumulative-score-mode=mean-zscore (0=only actual, 1=only NN, default: 0.5)",
    )
    parser.add_argument(
        "--adaptive-nn-weight",
        action="store_true",
        default=False,
        help="Enable adaptive NN weight scheduling: early turns trust EXP3, late turns trust NN",
    )
    parser.add_argument(
        "--adaptive-nn-weight-warmup",
        type=int,
        default=5,
        help="Midpoint turn where nn_weight ≈ 0.5 (default: 5)",
    )
    parser.add_argument(
        "--adaptive-nn-weight-scale",
        type=float,
        default=2.0,
        help="Controls transition sharpness for adaptive nn_weight (larger = smoother, default: 2.0)",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default=False,
        help=(
            "Enable UCB-style exploration bonus: score = score + ucb_c * sqrt(log(t) / (n_arm + 1)). "
    ...
    parser.add_argument(
        ...
        type=float,
        default=2.0,
        help="UCB exploration coefficient (default: 2.0). Only used when --ucb-exploration is enabled.",
    )
    parser.add_argument(
        "--seed",
        type=int,
    ...
    parser.add_argument(
        "--selection-strategy",
        type=str,
        choices=["greedy", "epsilon_greedy", "softmax", "round_robin", "fitness_weighted"],
        default=None,
        help=(
    ...
    parser.add_argument(
        "--strategy-version",
        type=str,
        choices=["v1", "v2", "v3", "v4", "s5", "s6", "s8", "s10", "s20", "s24", "s48", "s50", "s100"],
        default="v1",
        help="Strategy version: 'v1' (13), 'v2' (10), 'v3' (12), 'v4' (34 variants), or 's5/s10/s20/s50/s100' (generated spaces)",
    )
    parser.add_argument(
        "--output",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Suffix to append to auto-generated tag (e.g., 'ablation_eta_10')",
    )
    parser.add_argument(
        "--embeddings-dir",
        type=Path,
...
def parse_scenario_ids(scenario_ids_arg: str, embeddings_dir: str | Path | None = None) -> list[str]:
    """
    ...
    if embeddings_dir:
        ...
        if emb_path.exists():
            ...
            logger.info(f"Found {len(available_combo_ids)} combo IDs in embeddings directory: {emb_path}")
    ...
    for raw_id in raw_ids:
        ...
        if combo:
            if available_combo_ids is None or raw_id in available_combo_ids:
                ...
                logger.warning(f"Combo ID {raw_id} exists in DB but not in embeddings directory")
            continue

        # Treat as env_id: query database for matching combos
        if raw_id not in env_to_combo_cache:
            try:
                combos_for_env = list(EnvAgentComboStorage.find(
        ...
        if combo_ids_for_env:
            ...
            if available_combo_ids:
                matching = [cid for cid in combo_ids_for_env if cid in available_combo_ids]
                if matching:
                    logger.info(f"Env ID {raw_id} -> {len(matching)} combo(s) with embeddings: {matching}")
                    resolved_combo_ids.extend(matching)
                else:
                    logger.warning(f"Env ID {raw_id} has {len(combo_ids_for_env)} combos but none in embeddings directory")
            else:
    ...
    logger.info(f"Resolved {len(raw_ids)} input IDs to {len(resolved_combo_ids)} combo IDs")
...
async def run_batch_episodes(args: argparse.Namespace) -> None:
    ...
    # Append suffix if provided (for ablation studies)
    if args.tag_suffix:
    ...
    if args.embeddings_dir:
        ...
        embeddings_dir = Path(__file__).parent / "embeddings_backgrounds" / subset_for_embeddings
    ...
    if args.scenario_ids:
        ...
        scenario_ids = parse_scenario_ids(args.scenario_ids, embeddings_dir=embeddings_dir)
        console.print(f"[cyan]Using {len(scenario_ids)} resolved combo IDs from --scenario-ids[/]")

        if not scenario_ids:
            console.print("[red]Error: No valid combo IDs found. Check if:[/]")
            console.print(f"  - The input IDs exist in the database")
            if embeddings_dir:
                console.print(f"  - The embeddings directory contains matching combos: {embeddings_dir}")
        ...
        console.print(f"[green]Found {len(combos)} combos in database (strategy mode, no embeddings filtering)[/]")
    ...
    if args.continue_from:
        ...
        try:
            ...
            filtered_combos = [c for c in filtered_combos if c.pk not in completed_scenarios]
    ...
    if hasattr(args, "_extend_completed_scenarios") and args._extend_completed_scenarios:
    ...
    if previous_results:
        console.print(f"[cyan]Copying {len(previous_results)} previous results to new experiment...[/]")
...
async def run_multiple_scenarios(args: argparse.Namespace) -> None:
    ...
    for i, scenario_id in enumerate(scenario_ids, 1):
        ...
        try:
            ...
            results.append({"scenario_id": scenario_id, "status": "success", "summary": summary})
            successful += 1
            ...
            traceback.print_exc()
            results.append({"scenario_id": scenario_id, "status": "failed", "error": str(e)})
            failed += 1
    ...
    console.print(f"  Total: {len(scenario_ids)}, Success: {successful}, Failed: {failed}")
...
async def run_extend_from_hard(args: argparse.Namespace) -> None:
    ...
    console.print(f"[green]✓ Found {len(completed_scenarios)} completed scenarios to exclude[/]")
    ...
    console.print(f"  Excluding: {len(completed_scenarios)} already-completed scenarios")
    ...
    if run_config_file.exists():
        ...
        if original_selection_mode == "strategy":
            ...
            try:
                ...
                console.print(f"[cyan]Found {len(all_scenario_ids)} total scenarios for subset '{original_subset}' from database[/]")
            except Exception as e:
                logger.warning(f"Failed to get combos for subset '{original_subset}': {e}")
        ...
        if not all_scenario_ids:
            ...
            if embeddings_dir:
                ...
                if embeddings_path.exists():
                    ...
                    console.print(f"[cyan]Found {len(all_scenario_ids)} total scenarios from embeddings dir[/]")
...
async def main() -> None:
    ...
    if args.auto_proxy:
        console.print(f"[cyan]Starting mihomo proxy automatically (base port: {args.proxy_base_port})...[/]")
        ...
        try:
            ...
            console.print("[yellow]Do you want to continue without proxy? (y/N): [/]", end="")
            try:
                response = input().strip().lower()
    ...
    try:
        ...
        console.print(f"[yellow]Warning: Error closing LiteLLM clients (non-fatal): {e}[/]")
        import traceback
        traceback.print_exc()
