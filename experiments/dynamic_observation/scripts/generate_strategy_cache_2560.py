#!/usr/bin/env python3
"""
Pre-generate strategy embedding cache with custom embedding dimensions.

Usage:
    cd experiments/dynamic_observation
    # Standard 4096 dimension (8b model)
    uv run python scripts/generate_strategy_cache.py --subset hard --strategy-version v2 --cache-dir ./cache/strategy_embeddings_v2

    # 2560 dimension (4b model)
    uv run python scripts/generate_strategy_cache_2560.py --subset hard --strategy-version v2
"""
...
from experiments.dynamic_observation.core.envs_dynamic_parallel import get_bio, render_text_for_agent
from experiments.dynamic_observation.core.bandits.strategy_space import StrategySpace
...
def main():
    parser = argparse.ArgumentParser(description="Pre-generate strategy embedding cache (2560 dim)")
    parser.add_argument("--subset", type=str, default="hard", choices=["hard", "all"],
                        help="Dataset subset to process")
    parser.add_argument("--cache-dir", type=str, default="./cache/strategy_embeddings_v2_2560",
                        help="Directory to store cached embeddings")
    parser.add_argument("--embedding-model", type=str, default="qwen/qwen3-embedding-4b",
                        help="Embedding model to use (default: qwen/qwen3-embedding-4b for 2560 dim)")
    parser.add_argument("--embedding-dim", type=int, default=2560,
                        help="Embedding dimension (default: 2560)")
    parser.add_argument("--skip-existing", action="store_true",
                        help="Skip scenarios that already have cached embeddings")
    parser.add_argument("--strategy-version", type=str, default="v2", choices=["v1", "v2"],
                        help="Strategy version: v1 (13 strategies) or v2 (10 quadrant-based strategies)")
    ...
    console.print(f"[bold]Generating strategy embedding cache (2560 dim)[/bold]")
